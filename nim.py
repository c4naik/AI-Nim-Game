...
class Nim():
	def __init__(self, board = [9, 6, 6, 5, 3, 2]):   #Board la initialize karayche with coins
		self.board = board
		self.mode = 3
		self.player1 = 1
		self.player2 = 2
		self.winner = None
		self.playerMovesHistory = list()          #Moves print karayla, displayPreviousmove
		self.aiMovesHistory = list()
		self.gameDisplay = GUI()
		self.menuHeight = 500
		self.menuWidth = 500                      #500 pixel
		self.Player1Name = "Player 1"
		self.Player2Name = "Player 2"

	def setMode1(self, PvsAIMenu):			  #called in menuSetup
		global endGame
		endGame = False
		self.mode = 1
		self.saveNames(PvsAIMenu)		  #To set player1 and player2 name
		play(game)
		endGame = True

	def setMode2(self, PvsAIEMenu):
		global endGame
		endGame = False
		self.mode = 2
		self.saveNames(PvsAIEMenu)
		play(game)
		endGame = True

	def setMode3(self, PvsPMenu):
		global endGame
		endGame = False
		self.mode = 3
	...
	def menuSetup(self):
		...
		#create player vs Easy AI
		PvsAIEMenu = pygame_menu.Menu(
			height = self.menuHeight,
			width = self.menuWidth,
			title = "Enter Player Name",
			enabled = True,
		...
		PvsAIEMenu.add.text_input(
			"Player Name : ",
			default = "Human",
			textinput_id = "playerName"
			)

		PvsAIEMenu.add.button("Play", self.setMode2, PvsAIEMenu)

		PvsAIEMenu.add.button("Back", pygame_menu.events.BACK)

		PvsAIEMenu.add.button("Quit", pygame_menu.events.EXIT)
		...
		PvsPMenu.add.button("Play", self.setMode3, PvsPMenu)
		...
		modeMenu.add.button("Player vs AI (Difficult Mode)", PvsAIMenu)

		modeMenu.add.button("Player vs AI (Easy Mode)", PvsAIEMenu)

		modeMenu.add.button("Player vs Player", PvsPMenu)
	...
	def drawGame(self):
		...
		for pile in self.board:
			...
			for _ in range(pile):							#We are not interested in values returned
	...
	def remove(self, currentPlayer):
		...
		colorActive = (255, 0, 0)		#red
		colorPassive = (204, 204, 204)		#grey
		...
		while True:
			...
			self.gameDisplay.write(self.gameDisplay.height // 3, self.gameDisplay.font.get_height() * 3, "Row", (255, 255, 0))	#yellow
			...
			if submit.button.collidepoint(position):
				...
				submit.draw((51, 153, 255)) #dark blue
			...
			if finalrow is not None and finalamount == None:
				self.gameDisplay.write((self.gameDisplay.height // 2), self.gameDisplay.font.get_height() * 2, finalrow, (204, 0, 0)) #red
...
while True:
	...
	mainTheme.background_color = (255, 244, 153, 100)	#alpha channel(opacity) = 100,yellow
	mainTheme.title_background_color = (0, 0, 0, 100)
	mainTheme.title_font_color = (255, 255, 255)
	mainTheme.widget_font_color = (200, 200, 200)		#grey
	mainTheme.selection_color = (0, 0, 0)			#black
